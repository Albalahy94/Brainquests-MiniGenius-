...
import 'quick_math_game_screen.dart';

class QuickMathSelectScreen extends StatefulWidget {
  const QuickMathSelectScreen({super.key});

  @override
  State<QuickMathSelectScreen> createState() => _QuickMathSelectScreenState();
}

class _QuickMathSelectScreenState extends State<QuickMathSelectScreen> {
  ...
  void initState() {
    ...
    _storageService.init().then((_) {
      if (mounted) {
        setState(() {
          _progress = _storageService.getOrCreateGameProgress('quick_math');
        });
  ...
  void _startLevel(int levelNumber) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => QuickMathGameScreen(
          level: levelNumber,
