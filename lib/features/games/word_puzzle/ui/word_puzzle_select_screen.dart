...
import 'word_puzzle_game_screen.dart';

class WordPuzzleSelectScreen extends StatefulWidget {
  const WordPuzzleSelectScreen({super.key});

  @override
  State<WordPuzzleSelectScreen> createState() => _WordPuzzleSelectScreenState();
}

class _WordPuzzleSelectScreenState extends State<WordPuzzleSelectScreen> {
  ...
  void initState() {
    ...
    _storageService.init().then((_) {
      if (mounted) {
        setState(() {
          _progress = _storageService.getOrCreateGameProgress('word_puzzle');
        });
  ...
  void _startLevel(int levelNumber) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => WordPuzzleGameScreen(
          level: levelNumber,
