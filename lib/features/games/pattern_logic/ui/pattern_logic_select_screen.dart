...
import 'pattern_logic_game_screen.dart';

class PatternLogicSelectScreen extends StatefulWidget {
  const PatternLogicSelectScreen({super.key});

  @override
  State<PatternLogicSelectScreen> createState() =>
      _PatternLogicSelectScreenState();
}

class _PatternLogicSelectScreenState extends State<PatternLogicSelectScreen> {
  ...
  void initState() {
    ...
    _storageService.init().then((_) {
      if (mounted) {
        setState(() {
          _progress = _storageService.getOrCreateGameProgress('pattern_logic');
        });
  ...
  void _startLevel(int levelNumber) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => PatternLogicGameScreen(
          level: levelNumber,
