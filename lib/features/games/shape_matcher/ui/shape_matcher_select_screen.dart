...
class ShapeMatcherSelectScreen extends StatefulWidget {
  const ShapeMatcherSelectScreen({super.key});

  @override
  State<ShapeMatcherSelectScreen> createState() =>
      _ShapeMatcherSelectScreenState();
}

class _ShapeMatcherSelectScreenState extends State<ShapeMatcherSelectScreen> {
  ...
  void initState() {
    ...
    _storageService.init().then((_) {
      if (mounted) {
        setState(() {
          _progress = _storageService.getOrCreateGameProgress('shape_matcher');
        });
  ...
  void _startLevel(int levelNumber) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => ShapeMatcherGameScreen(
          level: levelNumber,
